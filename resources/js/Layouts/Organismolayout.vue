    ...
    },
    {
        ...
        items: [
            ...
            },
            {
                ...
                ruta:  'organismo.evidencias.index',
...
<template>
    <div class="layout-root">
        ...
        <div class="layout-body">
            ...
            <aside class="sidebar" :class="{ 'sidebar--collapsed': !sidebarAbierto }">
                ...
                <nav class="sidebar-nav">
                    <template v-for="seccion in menu" :key="seccion.seccion">
                        ...
                        <Link
                            ...
                            class="sidebar-item"
                            :class="{
                                'sidebar-item--active': esActiva(item.ruta),
                                'sidebar-item--col':    !sidebarAbierto,
                            }"
                ...
                <div class="sidebar-footer">
                    <button
                        ...
                        :class="{ 'sidebar-logout--col': !sidebarAbierto }"
                        @click="cerrarSesion"
                        title="Cerrar sesión"
                    >
            ...
            <!-- Contenido principal — slot para las páginas -->
...
.banner { position:fixed;top:0;left:0;right:0;height:var(--banner-h);z-index:110;background:var(--color-arena);border-bottom:2px solid rgba(174,25,34,0.15);box-shadow:var(--shadow-sm);display:flex;align-items:center;justify-content:center;overflow:hidden;padding:0; }
.banner-img { height:100%;width:100%;object-fit:cover;object-position:center;display:block;margin:0 auto; }

.layout-body { display:flex;margin-top:var(--banner-h);min-height:calc(100vh - var(--banner-h)); }

.sidebar { position:fixed;top:var(--banner-h);left:0;bottom:0;width:var(--sidebar-w);background:#0d2b27;display:flex;flex-direction:column;transition:width var(--transition-slow);z-index:90;overflow:hidden; }
...
.sidebar-user { display:flex;align-items:center;gap:.75rem;padding:1rem .85rem;border-bottom:1px solid rgba(255,255,255,0.07);flex-shrink:0;min-height:72px; }
.sidebar-user--col { justify-content:center;padding:1rem 0; }
.sidebar-avatar { width:36px;height:36px;border-radius:var(--radius-md);background:var(--color-verde);font-size:var(--text-xs);font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;letter-spacing:.05em;color:white; }
...
.sidebar-nav { flex:1;overflow-y:auto;overflow-x:hidden;padding:.75rem 0;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,0.08) transparent; }
.sidebar-seccion { font-size:var(--text-xs);font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:rgba(255,255,255,0.22);padding:.9rem .85rem .3rem;white-space:nowrap; }
.sidebar-divider { height:1px;background:rgba(255,255,255,0.07);margin:.6rem .85rem; }

.sidebar-item { display:flex;align-items:center;gap:.7rem;padding:.55rem .85rem;margin:1px .5rem;border-radius:var(--radius-md);text-decoration:none;color:rgba(255,255,255,0.5);font-size:var(--text-sm);font-weight:500;transition:background var(--transition-base),color var(--transition-base);white-space:nowrap;overflow:hidden;border-left:2px solid transparent; }
.sidebar-item--col { justify-content:center;padding:.55rem 0;margin:1px .4rem; }
...
.sidebar-footer { padding:.75rem .5rem;border-top:1px solid rgba(255,255,255,0.07);flex-shrink:0; }
.sidebar-logout { display:flex;align-items:center;gap:.7rem;width:100%;padding:.55rem .85rem;border-radius:var(--radius-md);background:none;border:none;cursor:pointer;color:rgba(255,255,255,0.35);font-size:var(--text-sm);font-weight:500;font-family:var(--font-body);transition:background var(--transition-base),color var(--transition-base);white-space:nowrap; }
.sidebar-logout--col { justify-content:center;padding:.55rem 0; }
