...
const form    = useForm({ plazo: '', activo: true })
...
const guardar = () => {
    if (modal.value === 'crear') {
        form.post(route('admin.catalogos.plazos.store'), { onSuccess: cerrar })
    } else {
        form.put(route('admin.catalogos.plazos.update', actual.value.id), { onSuccess: cerrar })
    }
}

const confirmarEliminar = () => {
    router.delete(route('admin.catalogos.plazos.destroy', actual.value.id), { onSuccess: cerrar })
}

const busqueda = ref('')
const filtrados = computed(() =>
    props.plazos.filter(i =>
        i.plazo.toLowerCase().includes(busqueda.value.toLowerCase())
    )
)
</script>
    ...
    <AdminLayout>
        <div class="page">
            ...
            <div class="search-bar">
                ...
                <span class="search-count">{{ filtrados.length }} de {{ plazos.length }}</span>
            </div>
            ...
            <div class="tabla-wrap">
                <table class="tabla">
                    ...
                    <tr v-for="item in filtrados" :key="item.id" class="tr">
                        ...
                        <td class="td td--nombre">{{ item.plazo }}</td>
                ...
                <div class="modal">
                    ...
                    <div class="modal-body">
                        <div class="field">
                            ...
                            <input v-model="form.plazo" type="text" class="field-input" :class="{ 'field-input--error': form.errors.plazo }" placeholder="Ej. Corto plazo, Mediano plazo, Largo plazo" @keyup.enter="guardar" autofocus/>
                            <p v-if="form.errors.plazo" class="field-error">{{ form.errors.plazo }}</p>
                        </div>
            ...
            <div v-if="modal === 'eliminar'" class="modal-overlay" @click.self="cerrar">
                <div class="modal modal--sm">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="delete-msg">¿Eliminar <strong>"{{ actual?.plazo }}"</strong>?</p>
