...
const form    = useForm({ estrategia: '', activo: true })
...
const abrirEditar = (item) => {
    form.estrategia = item.estrategia
...
const guardar = () => {
    if (modal.value === 'crear') {
        form.post(route('admin.catalogos.estrategias.store'), { onSuccess: cerrar })
    } else {
        form.put(route('admin.catalogos.estrategias.update', actual.value.id), { onSuccess: cerrar })
    }
}

const confirmarEliminar = () => {
    router.delete(route('admin.catalogos.estrategias.destroy', actual.value.id), { onSuccess: cerrar })
}

const busqueda = ref('')
const filtrados = computed(() =>
    props.estrategias.filter(i =>
        i.estrategia.toLowerCase().includes(busqueda.value.toLowerCase())
    )
)
</script>
    ...
    <AdminLayout>
        <div class="page">
            ...
            <div class="search-bar">
                ...
                <span class="search-count">{{ filtrados.length }} de {{ estrategias.length }}</span>
            </div>
            ...
            <div class="tabla-wrap">
                <table class="tabla">
                    ...
                    <tr v-for="item in filtrados" :key="item.id" class="tr">
                        ...
                        <td class="td td--nombre">{{ item.estrategia }}</td>
                ...
                <div class="modal">
                    ...
                    <div class="modal-body">
                        <div class="field">
                            ...
                            <input v-model="form.estrategia" type="text" class="field-input" :class="{ 'field-input--error': form.errors.estrategia }" placeholder="Ej. Fortalecimiento institucional" @keyup.enter="guardar" autofocus/>
                            <p v-if="form.errors.estrategia" class="field-error">{{ form.errors.estrategia }}</p>
                        </div>
            ...
            <div v-if="modal === 'eliminar'" class="modal-overlay" @click.self="cerrar">
                <div class="modal modal--sm">
                    ...
                    <div class="modal-body">
                        ...
                        <p class="delete-msg">¿Eliminar <strong>"{{ actual?.estrategia }}"</strong>?</p>
