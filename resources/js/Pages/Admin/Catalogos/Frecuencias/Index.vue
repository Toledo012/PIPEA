...
const form    = useForm({ frecuencia: '', activo: true })
...
const abrirEditar = (item) => {
    form.frecuencia = item.frecuencia
...
const guardar = () => {
    if (modal.value === 'crear') {
        form.post(route('admin.catalogos.frecuencias.store'), { onSuccess: cerrar })
    } else {
        form.put(route('admin.catalogos.frecuencias.update', actual.value.id), { onSuccess: cerrar })
    }
}

const confirmarEliminar = () => {
    router.delete(route('admin.catalogos.frecuencias.destroy', actual.value.id), { onSuccess: cerrar })
}

const busqueda = ref('')
const filtrados = computed(() =>
    props.frecuencias.filter(i =>
        i.frecuencia.toLowerCase().includes(busqueda.value.toLowerCase())
    )
)
</script>
        ...
        <div class="page">
            ...
            <div class="page-head">
                <div>
                    ...
                    <h1 class="page-title">Frecuencias</h1>
                </div>
            ...
            <div class="search-bar">
                ...
                <span class="search-count">{{ filtrados.length }} de {{ frecuencias.length }}</span>
            </div>
                ...
                <table class="tabla">
                    ...
                    <tr v-if="filtrados.length === 0">
                        <td colspan="4" class="tabla-empty">
                            <p>{{ busqueda ? `Sin resultados para "${busqueda}"` : 'No hay registros.' }}</p>
                            <button v-if="!busqueda" class="btn btn-secondary btn-sm" @click="abrirCrear">Agregar primero</button>
                        </td>
                    </tr>
                    <tr v-for="item in filtrados" :key="item.id" class="tr">
                        <td class="td td--id">{{ item.id }}</td>
                        <td class="td td--nombre">{{ item.frecuencia }}</td>
                        <td class="td td--center">
                                <span :class="['badge-estado', item.activo ? 'badge-estado--activo' : 'badge-estado--inactivo']">
                                    {{ item.activo ? 'Activo' : 'Inactivo' }}
                                </span>
                        </td>
                        <td class="td td--center td--acciones">
                            <button class="btn-ico btn-ico--edit" @click="abrirEditar(item)" title="Editar">
                            ...
                            <button class="btn-ico btn-ico--delete" @click="abrirEliminar(item)" title="Eliminar">
            ...
            <div v-if="modal === 'crear' || modal === 'editar'" class="modal-overlay" @click.self="cerrar">
                <div class="modal">
                    <div class="modal-head">
                        <h2 class="modal-title">{{ modal === 'crear' ? 'Nuevo registro' : 'Editar registro' }}</h2>
                    ...
                    <div class="modal-body">
                        <div class="field">
                            ...
                            <input v-model="form.frecuencia" type="text" class="field-input" :class="{ 'field-input--error': form.errors.frecuencia }" placeholder="Ej. Mensual, Trimestral, Semestral, Anual" @keyup.enter="guardar" autofocus/>
                            <p v-if="form.errors.frecuencia" class="field-error">{{ form.errors.frecuencia }}</p>
                        </div>
                    ...
                    <div class="modal-foot">
                        ...
                        <button class="btn btn-primary" @click="guardar" :disabled="form.processing">
                            ...
                            {{ form.processing ? 'Guardando...' : (modal === 'crear' ? 'Crear' : 'Guardar') }}
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div v-if="modal === 'eliminar'" class="modal-overlay" @click.self="cerrar">
                <div class="modal modal--sm">
                    <div class="modal-head modal-head--danger">
                        <h2 class="modal-title modal-title--danger">Eliminar registro</h2>
                    ...
                    <div class="modal-body">
                        ...
                        <p class="delete-msg">¿Eliminar <strong>"{{ actual?.frecuencia }}"</strong>?</p>
...
.td--id { color: var(--color-gris-400); font-size: var(--text-xs); font-weight: 600; }
...
.modal { background: var(--color-blanco); border-radius: var(--radius-xl); box-shadow: 0 20px 60px rgba(0,0,0,0.2); width: 100%; max-width: 440px; animation: modal-in 0.18s ease; }
